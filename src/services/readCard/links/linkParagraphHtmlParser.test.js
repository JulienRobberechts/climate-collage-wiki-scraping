...
const linksCausesFilePathForCard = (cardNum) =>
  `./src/services/readCard/links/test-data/fr/linksCauses${cardNum}.html`;

const linksEffectsFilePathForCard = (cardNum) =>
  `./src/services/readCard/links/test-data/fr/linksEffects${cardNum}.html`;
...
describe("parse other links", () => {
  it("parse optional Effects card 1", async () => {
    ...
    expect(links[0].href).toEqual(
      "/wiki/index.php?title=Fr-fr_adulte_carte_25_biodiversit%C3%A9_terrestre"
    );
  });
  ...
  it("parse optional Causes card 25", async () => {
    const content = await getContent(linksCausesFilePathForCard(25));
    const links = parseLinks(content);
